...
- Not built: captcha — isu #24 fase 4 chose per-`(ip, username)` rate limiting in Redis instead, a decision, not an oversight, see "Authentication and authorization" — and the daily reconciliation job over the balance chain. Self password change, logout, refresh tokens, and session revocation shipped in isu #24; an access token itself remains unrevocable by design, now bounded by `jwt.ttl_minutes` (15, down from 60) rather than by nothing
