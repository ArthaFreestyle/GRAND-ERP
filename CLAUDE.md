...
- Implemented: **`satuan`**, **`ekspedisi`**, **`supplier`**, **`pelanggan`**, **`role`**, **`user`** (create / get / list / patch), **`ruang`** (create / get / list, no patch), **`product`** with `product_satuan` + `product_harga_jual`, **`pembelian`** with `pembelian_detail`, `kartu_stok` posting, and `document_counter`, **`penerimaan_susulan`** with `penerimaan_susulan_detail`, **`retur_pembelian`** with `retur_pembelian_detail`, and **`riwayat_beli`** — a query, not a module
...
- The rest of the inventory/sales schema exists as migrations `000002`–`000008` and still has **no Go layers** — sales, sales returns, `pemakaian`, `mutasi`, stock opname, and both payment sides. See "Inventory data model" below
...
- **A return does not yet reduce the payable, and that is a deferral rather than an oversight.** `total` is derived from cost, which includes the freight share and whatever PPN treatment applied, while `pembelian.total` excludes freight entirely — so subtracting one from the other over-credits the supplier by money that went to the carrier. What the supplier is actually credited is fase 6's decision (`pembayaran_utang`); do not wire a `status_pembayaran` recompute from here without settling that first.
...
- ~~posted documents must reject detail edits~~ — done for `pembelian`, `penerimaan_susulan`, and `retur_pembelian`, via `kunciDenganStatus`. Still owed by every other document type
...
- **a POSTED return must reduce the payable.** Not built, and deliberately left to fase 6: `retur_pembelian.total` is derived from cost and includes the freight share, while `pembelian.total` excludes freight, so the two cannot be subtracted as they stand. See "Retur pembelian"
