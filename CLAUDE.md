...
- `cmd/worker` has two jobs, the orphan-attachment sweep and — isu #25 — the `kartu_stok` balance-chain reconciliation. Scheduler is a `time.Ticker` per job in `internal/config/worker.go`, wired by `BootstrapWorker` — the counterpart of `Bootstrap`, so "wiring happens only in `internal/config`" still holds. Both are daily, so neither forced a move to a cron library
