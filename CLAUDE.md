...
- The rest of the inventory/sales schema exists as migrations `000002`–`000008` and still has **no Go layers** — sales returns, stock opname, and receiving payments against a receivable (`penerimaan_pembayaran`). See "Inventory data model" below. The posting engine itself is finished: `mutasi` and `pemakaian` were the last shapes it was missing; `penjualan` is the last document type, not a new shape
...
- `PenjualanUseCase` carries no `RuangRepository`, following `pemakaian` rather than `pembelian`/`mutasi`: isu #10 does not ask for `id_ruang` to be validated against the caller's active `unit_kerja`, and this module does not add that scope on its own initiative.
