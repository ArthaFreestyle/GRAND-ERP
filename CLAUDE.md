...
Implemented: `satuan`, `ekspedisi`, `supplier`, `pelanggan`, `role`, `user`, `unit_kerja`, `ruang` (create/get/list/patch — isu #23 closed the missing PATCH), `periode` (list/get/tutup/buka), `product` (+ `product_satuan`, `product_harga_jual`), `dokumen`, and the transaction documents `pembelian`, `penerimaan_susulan`, `retur_pembelian`, `pembayaran_utang`, `mutasi`, `pemakaian`, `penjualan`, `penerimaan_pembayaran`, `stok_opname`. Isu #40 added `presensi` — attendance — the first module touching neither `kartu_stok` nor money, and the only slice whose write DTO carries no identity field at all. Ten reads that are **not** modules: `riwayat_beli`, `utang_supplier`, `stok_per_ruang`, `piutang_pelanggan`, — isu #22 — `kartu_stok_riwayat`, `stok_minimum`, `laporan_nilai_persediaan`, `laporan_laba_kotor`, `laporan_pergerakan`, and — isu #37 — `laporan_kesehatan_stok`.

`ruang.id_unit_kerja` is absent from `PATCH /api/v1/ruang/{id}`'s DTO — not imutability for its own sake the way `product.kode_barang` is, but because `kartu_stok` is partitioned by `(id_barang, id_ruang)` and isu #12 fase 6's read scoping reads a room's unit straight off that column. Moving a `ruang` to another unit through PATCH would carry its whole history and balance across with no `kartu_stok` row ever changing — the old unit's reports lose stock that was really there, the new one gains stock that never arrived through any document. A `ruang` assigned to the wrong unit is retired and re-created; goods that actually moved are a `mutasi` into a room the destination unit owns. Retiring a `ruang` (`is_aktif: false`) is refused 409 while it still holds stock (some `(barang, ruang)` with `stok_akhir > 0` — empty it with `mutasi` or `pemakaian` first) or while an open `stok_opname` freezes it (isu #15; the message names the freezing opname's own `nomor`, the same figure `RuangResponse.nomor_opname_beku` already carries).
...
| people, not goods or money | `presensi` | no `kartu_stok`, no `periode`, no number, no `big.Rat`; identity from the session with no DTO field at all; two unique indexes with deliberately different keys, one full and one partial |
...
- `cmd/worker` has three jobs: the orphan-attachment sweep, — isu #25 — the `kartu_stok` balance-chain reconciliation, and — isu #40 fase 5 — the `presensi` `LUPA_PULANG` sweep. Scheduler is a `time.Ticker` per job in `internal/config/worker.go`, wired by `BootstrapWorker` — the counterpart of `Bootstrap`, so "wiring happens only in `internal/config`" still holds. All three are daily, so none forced a move to a cron library
- Not built: captcha — isu #24 fase 4 chose per-`(ip, username)` rate limiting in Redis instead, a decision, not an oversight, see "Authentication and authorization". Self password change, logout, refresh tokens, and session revocation shipped in isu #24; an access token itself remains unrevocable by design, now bounded by `jwt.ttl_minutes` (15, down from 60) rather than by nothing
- Every master slice fills `created_by`/`updated_by` from `middleware.SessionFrom` now (isu #23) — `product` and `pembelian` were the first, and the pattern is `product_controller.go`'s: the controller reads the session and overwrites the DTO's `ActorID` (`json:"-"`) unconditionally after binding. `role` and `user` are `SUPERADMIN`-only so `created_by` is always present there except the seeded superadmin row itself, written straight to the database with no caller and `NULL` forever by design
...
- **Since isu #24, the same package also needs `TEST_REDIS_ADDR`** — `AuthUseCase.Login` itself hard-requires Redis now (refresh tokens, login throttling), which is not a new category of dependency: the running app already refuses to boot without Redis (`config.NewRedis` `Fatal`s on a failed ping). `newApp` gates on both; without `TEST_REDIS_ADDR` the package still compiles and non-auth-dependent tests still skip cleanly, same shape as missing `TEST_DATABASE_URL`. `flushAuthKeys` (`main_test.go`) clears `refresh:*`/`login_throttle:*` before every test via a targeted `SCAN`, never `FLUSHDB` — safe even if `TEST_REDIS_ADDR` accidentally points at a non-scratch instance.
...
- **Access tokens still cannot be revoked — isu #24 did not reopen that, only shrank the window it bounds.** Nothing about a JWT is stored server-side; `is_aktif = false`, a revoked role, or a revoked grant does not reach an issued access token. `jwt.ttl_minutes` (15, down from 60) is the entire residual bound. **Do not "fix" this with a Redis blacklist on the access token** — it reinstates the per-request lookup JWT was chosen to avoid. `switch-context` still mints a brand new access token and still cannot touch the old one.
- **Refresh tokens, unlike access tokens, are a stateful and revocable credential — isu #24 fase 2/3.** They are opaque random strings (`crypto/rand`, 256 bits, base64url), never JWTs, stored in Redis via `RefreshTokenRepository` (key `refresh:<token>` → `{user_id, id_user_role}`, plus a per-user index `refresh:user:<id>` re-`EXPIRE`d on every write so an abandoned session's index entry ages out on its own). `POST /api/v1/auth/refresh` consumes one atomically (`GETDEL`, closing the reuse race by construction) and mints a new access/refresh pair, re-resolving the active grant against the database rather than trusting the stored record — the same re-check `switch-context` already does. `POST /api/v1/auth/logout` deletes one, idempotently. Being opaque is what keeps a refresh token out of every protected route with no special-casing: `Authenticate` only ever tries to parse a bearer token as a JWT, so a refresh token handed to an ordinary route simply fails to parse.
- **Three things revoke every refresh token a user holds, via `RefreshTokenRepository.RevokeAllForUser`**: a password change (self-service `POST /api/v1/auth/me/password`, or `SUPERADMIN` resetting it through `PATCH /user/{id}`), `is_aktif` becoming `false`, and `grants` being replaced with `[]`. The effect is not instant — it lands the moment the caller's already-issued access token expires, ≤15 minutes with the current default — and that bound is stated in responses/docs, not hidden.
- **`POST /api/v1/auth/me/password` carries no role guard**, the same tier as `auth/me` and `switch-context` — a session with no active context may still secure its own account. `password_lama` is verified even though the caller already holds a valid session (a stolen access token must not be able to lock the real owner out by changing it out from under them), and a wrong one answers the identical `"username or password is wrong"` used everywhere else in this module, never a distinct message. This is a different action from `SUPERADMIN`'s `PATCH /user/{id}` password reset, which needs no `password_lama` — a manager legitimately does not know a subordinate's password.
- **`POST /api/v1/auth/refresh` and `POST /api/v1/auth/logout` carry no bearer requirement at all** — both exist specifically for the case where the access token has already expired, so requiring one would defeat the endpoint.
- **Login throttling (isu #24 fase 4) replaced captcha, a deliberate choice stated in the issue itself.** `LoginThrottleRepository` counts failures in Redis keyed `login_throttle:<ip>:<username>` (`throttle.login.max_attempts`/`window_minutes`, default 5 per 15 minutes) — **per pair, not per username**, so one attacker spamming a victim's username from their own IP cannot lock that victim out from the victim's own IP. A throttled attempt answers the identical message and status a wrong password gets (`bcryptDummyCompare` still runs, so timing does not leak which case fired) — distinguishing them would tell a client it is being rate-limited rather than simply wrong. A successful login resets the counter. `ctx.IP()` is used as-is, with no trusted-proxy configuration anywhere in this codebase — behind a reverse proxy every request can present the same address, weakening the per-pair split back toward per-username; not solved here.
...
- **Login answers one message for every failure** (unknown username, wrong password, disabled account, and now a throttled `(ip, username)` pair) — distinguishing any of them enumerates usernames or leaks throttling state. The unknown-username path runs a dummy bcrypt compare so it does not return measurably faster.
...
- Prefix `PM`.
- **`id_ruang` is validated against the caller's active unit_kerja, and reads are scoped by it** — isu #21 fase 2, added after isu #9 shipped without it. See "Unit kerja, active context, and scoping" fase 5/6.
...
- Prefix `PJ`.
- **`id_ruang` is validated against the caller's active unit_kerja, and reads are scoped by it** — isu #21 fase 2, added after isu #10 shipped without it. See "Unit kerja, active context, and scoping" fase 5/6.
...
- **`PATCH /api/v1/user/{id}` revokes every refresh token the target holds when the patch sets a new password, `is_aktif: false`, or `grants: []` — isu #24 fase 3.** Checked against the patch actually applied (`UserUseCase.Update`, after commit), not the raw request, so `is_aktif: true` on an already-active user or an absent `grants` key triggers nothing. Best-effort and logged only, never propagated as an error — the SQL write already committed by the time this runs. See "Authentication and authorization" for the other two triggers (self password change, and what "revoked" actually bounds to).
...
- **A roles-only patch still has to move `updated_at`** — that is `UserRepository.Touch`: writes no other column besides `updated_by` (isu #23), fires the trigger, and yields `sql.ErrNoRows` so an unknown id still answers 404. Without `updated_by` there, a grant revocation would be the one change to a user that leaves no trace of who did it — exactly the change that most needs one.
...
Answering not just *who* but *acting as what, and where*. All five load-bearing phases are built, plus the read-scoping piece of the optional phase 6, plus the two decisions fase 1 wrote down but left unimplemented — closed by isu #21. **Deferred, not overlooked: `users.id_ruang_default` and a role-as-snapshot column on documents** — isu #21 confirmed both stay that way, and wrote down what would make either worth building.
...
- **`document_counter` gets a per-unit series** — key `(prefix, id_unit_kerja, tahun, bulan)`. Implemented in migration `000025` (isu #21 fase 1), once two real outlets were about to start sharing one series: a number is a fact once it is on paper in a supplier's hands, and the key behind it cannot change under it after that.
...
**`document_counter` per unit kerja (migration `000025`, isu #21 fase 1).** A number is now `PREFIX/KODE/YYYY/MM/NNNN` for a unit with a kode, or the pre-#21 `PREFIX/YYYY/MM/NNNN` for the global series — never a mix, and never the unit's numeric id standing in for a missing kode.

- **Two indexes, not a wider primary key** — `document_counter_scoped_uidx` on `(prefix, id_unit_kerja, tahun, bulan)` plus the partial `document_counter_global_uidx` on `(prefix, tahun, bulan) WHERE id_unit_kerja IS NULL`, mirroring `user_role_grant_uidx` + `user_role_grant_global_uidx` from migration `000020` for the identical reason: a unique index does not constrain `NULL`, so the scoped index alone would let ten global rows share one `(prefix, tahun, bulan)`. Every row `document_counter` held before `000025` keeps `id_unit_kerja IS NULL` — the migration adds the column with no backfill, so those series become, and remain, the global one, continuing from their own `last_number` rather than resetting.
- **`DocumentCounterRepository.Next` is two statements, not one**, for the same reason `UserRepository.ReplaceRoles` is: one `INSERT ... ON CONFLICT` names exactly one arbiter index, and a `NULL` unit and a real one are governed by two different ones.
- **The unit a number is keyed to is always the room the document actually belongs to, never the caller's active unit_kerja.** `nomorDokumenUntukRuang` (`shared.go`) resolves it off `id_ruang` — the same field fase 5 validates — for the six modules that have one: `pembelian`, `pemakaian`, `penjualan`, `stok_opname` from their own request, `mutasi` from `id_ruang_asal` only (never `id_ruang_tujuan`, mirroring fase 5's asymmetry), and `penerimaan_susulan`/`retur_pembelian` from the parent `pembelian`'s `id_ruang` — they carry none of their own. `pembayaran_utang` and `penerimaan_pembayaran` have no room at all, so `nomorDokumenUntukUnitAktif` keys their number to the caller's own active unit_kerja instead; a global active context falls back to the global series, the natural reading of "no unit" either way.
- **A unit with no kode cannot issue a number, refused before `Next` is ever called** — `nomorDokumen` (`shared.go`) checks first so a missing kode never burns a reserved number. The refusal names the unit's id rather than silently formatting the number with it.
- **`unit_kerja.kode` was already nullable and `PATCH`-able** before this issue (migration `000019`), so no schema change was needed there — only the refusal to build a number without one.
...
- **`pembelian`, `mutasi`, `stok_opname`, `pemakaian`, and `penjualan` have `id_ruang` in their own request body, so only they check it.** `penerimaan_susulan`/`retur_pembelian` copy it from the parent, whose `Create` already validated it. `pembelian`'s `PATCH` has no `id_ruang` field; `mutasi` needs it on `Create` **and** `Update` (both rooms may change while `DRAFT`); `stok_opname` only on `Create`; `pemakaian` and `penjualan` need it on both `Create` and `Update`, and only when the patch actually touches `id_ruang` — mirroring `mutasi`'s own `Update` rule, since neither has a second room to complicate the check.
...
- **`pemakaian` and `penjualan` opted out of both fase 5 and fase 6 when isu #9/#10 shipped, and that was the right call at the time** — neither issue asked for the scope, and a second outlet was not yet running. Isu #21 fase 2 closed that gap: once a second outlet is real, an unscoped `id_ruang` on either module is an authorization hole, not a missing convenience — a cashier from unit A could otherwise post a request or a nota against unit B's room with nothing to refuse it. Both now follow `mutasi`'s own shape exactly: checked on `Create` and, only when `id_ruang` is present in the patch, on `Update` too.

#### Fase 6 — read-path scoping (no migration)

Built after being explicitly asked for. Scoped: `ruang`, `pembelian`, `penerimaan_susulan`, `retur_pembelian`, `mutasi`, `stok_opname`, `product/{id}/stok`, and — since isu #21 fase 2 — `pemakaian` and `penjualan`.
...
- **Every scoped module gained one column alongside what it already read, never a second query** — an unexported `IDUnitKerjaRuang int64` on the entity, joined into the query that already joins `ruang` for its name. One column added to each `xReadColumns`, one scan target added to each `scanXRead`. `mutasi`'s is named `IDUnitKerjaRuangAsal` and comes from `asal.id_unit_kerja`. `pemakaian` and `penjualan` follow the same shape (isu #21 fase 2).
...
- **A filter clause reaching a joined table forces the `COUNT` query to join it too**, and four modules had to change for exactly that: each `Search` had been running `COUNT` against a bare `FROM table` that never reached `ruang`. Changing it to use the same `xFrom` constant as the row query is what "write the filter once, both queries use it" actually requires. `mutasi` needed the same fix even though `mutasiFrom` already joined both rooms for their names. `pemakaian` and `penjualan` needed no such fix when isu #21 fase 2 scoped them — both `Search` methods already ran `COUNT` against their own `xFrom` constant, so adding the unit clause to `xFilter` was enough on its own.
...
- Tests live in `fase6_read_scope_test.go`, one file spanning every scoped module — `pemakaian` and `penjualan` joined it too rather than getting files of their own, carrying both their write-side (fase 5) and read-side (fase 6) assertions together, plus one write-path re-read guard: an `Update` that never touches `id_ruang` must not be scoped by `AktifIDUnitKerja` in its own response, the same way every write-path call to `detail()` already passes `nil` regardless of who is calling.
...
### Rekonsiliasi rantai kartu_stok (isu #25, no migration)

The worker's second job. `kartu_stok` is append-only and every balance in it is trigger-computed, so the chain **should** always be internally consistent — this job is what actually checks that daily, instead of trusting the "should".

- **Reports, never repairs.** A discrepancy means a bug — in the trigger, a migration, or something that wrote to `kartu_stok` outside it — and there is no honest way to fix it from here: `UPDATE`/`DELETE` are rejected by `kartu_stok_append_only`, disabling that trigger to force one is a licence only the test database's `truncateMaster` gets, and inventing a new adjustment row would book a movement that never happened with no document behind it. If a product genuinely needs correcting, the tool is `stok_opname`, not this job.
- **`KartuStokRepository.PeriksaRantai` is one SQL statement**, not a page-by-page walk in Go — window functions (`LAG` over `(id_barang, id_ruang)` ordered by `id`, never by date) recompute the same three things `kartu_stok_hitung_saldo` does per row and compare against what is actually stored. Comparing `NUMERIC` columns directly in SQL, rather than pulling them into Go, is what keeps a `float64` out of it entirely — PostgreSQL's own `ROUND` compared against PostgreSQL's own `ROUND` cannot disagree with itself.
- **The upper `id` bound is a named CTE (`batas`), taken inside the same statement.** A single SQL statement executes against one MVCC snapshot, so `batas`'s `MAX(id)` and every row the window function reads afterward agree on exactly the same cutoff — a row committed after the statement started cannot appear as a disconnected tail in a partition the query already walked. Naming it explicitly keeps that guarantee legible to whoever reads the query next rather than true only by accident of today's implementation being one statement.
- **Only the immediately preceding stored row is ever consulted** (via `LAG`), never a value recomputed further back in the chain — the same thing the trigger itself does on every insert (`ORDER BY id DESC LIMIT 1` against whatever is already on disk). A single corrupted row is therefore reported once, at the row where the stored value first disagrees with its predecessor, not again on every row after it that the trigger correctly built on top of the bad one.
- **What is checked, and why three separate comparisons rather than one**: `stok_awal` against the previous row's stored `stok_akhir`; `stok_akhir` against `stok_awal + stok_masuk − stok_keluar`; and `harga_pokok_satuan`/`nilai_keluar`/`nilai_akhir` against the moving average, with `nilai_akhir` forced to exactly zero whenever the recomputed `stok_akhir` is zero. Each has a different root cause, so collapsing them into one boolean would erase which one actually broke.
- **No `is_aktif` filtering anywhere** — a retired product or room still holds history and value, and a sweep that skipped them would never notice damage sitting in exactly the rows a report like `laporan/nilai-persediaan` still reads.
- **Session-level `pg_advisory_lock`, on a `*sql.Conn`**, the same shape and the same trap `DokumenUseCase.BersihkanYatim` already has: releasing it from a different pooled connection does nothing, and a second worker finding the lock held simply has nothing to do this run.
- **Returns the count of broken chains, following `Job.Jalankan`'s contract.** Zero is logged at `Info` and is the expected, silent daily answer. Any discrepancy is logged at `Error` **per broken chain**, naming `id_barang`, `id_ruang`, and the id of the first row that diverges — enough to open that product's kartu stok directly, not just a number saying something, somewhere, is wrong.
- **Full daily sweep, on purpose, with no partial-scan mode.** Cost grows linearly with the table, and while it stays small a full walk every day is the honest answer; a mode limited to recently-touched partitions would never look at old rows again and could never find old damage. `rekonsiliasi.interval` is the only config key this issue adds, worker-only — `cmd/web` never runs this job and so never reads it.

### Presensi karyawan (migration 000028, isu #40)

Who came in, at what hour, in which shift — and, once closed, who left and when. **The first module here touching neither `kartu_stok` nor money** — no `periode`, no room freeze, no `document_counter`, no `big.Rat`, no posting engine — so what it keeps from this project's habits is only what still applies: identity from the session, vocabularies guarded by CHECKs, a derived figure never set from a form, and every correction leaving a trail. Master-slice shaped (`supplier`); what it borrows from the document side is exactly one idea, the unique index that answers a question no application check can.

- **There is a clock-out, and a forgotten one is never invented — that tension is the shape of the module.** `Masuk` opens a row (`status = 'BUKA'`); `Pulang` closes the one open row this user has, wherever it is. A shift that closes without a tap is `LUPA_PULANG` with `jam_pulang` left `NULL` forever — never a guessed hour (shift end time, `tanggal + 10 jam`, last tap seen). A made-up clock-out reads like a fact while being a guess, and in a module whose downstream is payroll a guess becomes rupiah.
- **`shift` is inferred by the server from the *clock-in* tap time, never chosen and never re-derived at clock-out.** One named constant, `batasShiftWIB = 17:30`, in `presensi_shift.go`: before it `PAGI`, from it `MALAM`. Pure and database-free with an internal test (`package usecase`), the `pembelian_alokasi.go` / `kesehatan_stok_skor.go` shape — **this boundary moves money** (17:29 is one shift, 17:31 another), so it is one place, tested without a fixture, and only the server's own clock can shift it. `Pulang` never compares its own tap time against a shift's nominal end and never re-infers — lembur and a late clock-out are not this module's to judge.
- **One boundary, not two windows.** The real hours (`PAGI` 07:00–17:00, `MALAM` 18:00–21:00) leave three gaps — before seven, the 17:00–18:00 break, after nine — and each gap is a tap that would have to be refused while the person is genuinely standing there. A single boundary has no gaps. Accepted knowingly: 03:00 records a very early `PAGI`, 23:00 a very late `MALAM`; both are recorded, and correction exists for the ones genuinely in the wrong shift.
- **Two unique indexes, two different keys, and swapping them breaks the module in both directions.** `presensi_user_tanggal_shift_uidx` is `(id_user, tanggal, shift)` — `shift` **must** be in this key, or whoever works both shifts is refused at the second tap and the day worth the most to payroll becomes the one day that cannot be recorded. `presensi_terbuka_uidx` is a **partial** unique on `(id_user) WHERE status = 'BUKA'` — `shift` **must not** be in this key, because a person cannot physically be in two shifts at once; the index is what makes "which row does Pulang close" have exactly one answer, and what makes `Masuk` able to detect "this user still has something open" without a scan.
- **A tap for a slot already occupied — `BUKA` or `SELESAI` — answers 409, never a second row and never a silent 200.** A phone on a bad connection sends the same tap twice, and the answer has to let the client draw the right state without pretending it just recorded something. The Go pre-check (`tolakJikaSlotTerisi`) is for the message only; the unique index is the guarantee, per "check-then-insert never guarantees uniqueness". Two different indexes can reject the `INSERT` that follows, and the message distinguishes them by constraint name: `presensi_user_tanggal_shift_uidx` means this slot was taken by a concurrent tap, `presensi_terbuka_uidx` means a concurrent tap already opened a different shift for this same user.
- **`id_user` is not `json:"-"`-then-overwritten the way `ActorID` is — the field does not exist in any DTO.** `HadirRequest`/`PulangRequest` bind no body at all. If a client could send `id_user`, one employee could mark a colleague present who has not reached the office (or clock them out), and the module loses its reason to exist.
- **`id_unit_kerja` is a snapshot of the session's active grant at `Masuk` time, never re-taken at `Pulang`.** Deriving it on read would rewrite history: a grant can move to another unit next month, and last month's recap would move with it. A session with no active context (`Aktif == nil`) may still tap — the row simply carries NULL, and consequently is visible only to a caller who is themselves global. **Do not "fix" that by picking the first grant the person holds**; isu #12 fase 4 already decided not to guess a default grant.
- **`GET /presensi/saya/hari-ini` answers BOTH shifts at once**, with a synthetic `status: "BELUM_MASUK"` for a shift with no row rather than a 404 — the shape `periode` gives an unclosed month. It carries `shift_sekarang` because **the server decides which button to draw**: a client inferring the shift from a phone's clock puts people in the wrong shift whenever it drifts, and around 17:30 a few minutes is enough.
- **`durasi_kerja_menit` is the one derived figure in this project deliberately NOT frozen as a snapshot.** Every money figure elsewhere is a snapshot because its source moves (a moving average, a running receivable); here both operands (`jam_masuk`, `jam_pulang`) live on the same row a correction can still move, so computing it at read time is what keeps it from silently disagreeing with the two columns beside it. `BUKA` and `LUPA_PULANG` answer `null`, never `0` — the `stok_so = NULL` vs `0` distinction from `stok_opname`.
- **The recap reports and does not decide.** Grouped per `(id_user, bulan)` with the shifts kept apart: `hari_pagi_selesai`, `hari_malam_selesai`, `hari_dua_shift` (both shifts `SELESAI` the same calendar day), `hari_lupa_pulang_pagi`/`hari_lupa_pulang_malam` (counted separately, never folded into the `SELESAI` counts), `total_menit_kerja_pagi`/`total_menit_kerja_malam`. `hari_dua_shift` is its own number — `hari_pagi_selesai + hari_malam_selesai` counts that day twice and cannot say which days overlapped — computed as a nested aggregation (`COUNT` over a `GROUP BY (id_user, tanggal)` CTE) in **one query per page**, the `SaldoPerRuangBatch` anti-N+1 shape. **Whether a `LUPA_PULANG` day is payable is payroll policy and is deliberately not answered here**; folding it into `hari_dua_shift` now locks a pay policy in before it is written.
- **Corrections leave a trail through one endpoint, `PATCH /api/v1/presensi/{id}`, `SUPERADMIN`.** There is no separate "type in a missed tap" endpoint in this revision — every row starts life through `Masuk`/`Pulang` or through the sweep, and PATCH only ever moves an hour, a clock-out, or a shift that already exists. It moves `jam_masuk`/`jam_pulang`/`shift`, writes `dikoreksi_oleh`, `ts_koreksi`, and a **required** `alasan_koreksi` that may not be cleared — the only record of why someone's hour changed. Policy, not constraint (the `keterangan_selisih` / `retur_pembelian.alasan` precedent), backed by `presensi_koreksi_check` making the actor and the reason all-or-nothing.
- **Only the touched column's own `sumber_*` turns `KOREKSI`.** Correcting `jam_masuk` or `shift` marks `sumber_masuk`; correcting `jam_pulang` marks `sumber_pulang`. The two are tracked apart because a correction may touch only one side of the shift — filling in a `LUPA_PULANG` row's `jam_pulang` should not retroactively claim the person's own clock-in tap was typed by someone else.
- **`jam_pulang` may be filled or moved, but a PATCH may never null it back out.** Reopening a `SELESAI` shift would reactivate `presensi_terbuka_uidx` for a day already gone, and the person could not clock in again tomorrow. Refused in Go with `model.Invalid` before it ever reaches the database.
- **`status` is recomputed on every PATCH, never accepted from the form** — the `status_pembayaran` rule applied here. If the patch leaves `jam_pulang` non-`NULL` (already filled, or just filled by this patch), `status = 'SELESAI'`; otherwise `status` is untouched (`BUKA` or `LUPA_PULANG` stays as it was). Correcting a `LUPA_PULANG` row by filling `jam_pulang` therefore becomes `SELESAI` on its own, which is exactly what makes that day start counting toward `hari_dua_shift` once both shifts close.
- **`shift` is correctable but never re-inferred from a corrected `jam_masuk`.** Two separate decisions: tying them means fixing a one-minute typo silently moves someone into the other shift. A `shift` correction can collide with the unique index → `conflictOnUnique` → 409.
- **`jam_masuk`/`jam_pulang` are written as a wall clock in WIB (`"06:45"`), applied to the row's own `tanggal`** — which is what keeps a correction on its own day without a rule saying so (`id_user` and `tanggal` are absent from the update DTO; moving attendance to another person or day is not a correction). There is no CHECK behind the `tanggal`/`jam_masuk` pair to catch a disagreement, because `timestamptz::date` depends on the session's TimeZone and is not immutable enough to appear in one.
- **`zonaWIB` lives in `shared.go` and both callers use it** — this module and `tanggalHargaJual`. Two identical zone definitions in two files is the `periodeLockKey` duplication shape: one gets changed, the other silently does not. Here the zone decides **two** things, the calendar date and the shift, so one wrong definition is wrong twice.
- **`PresensiUseCase.Now` is a seam over `time.Now`, set in the constructor and overridable only by tests.** It is never a client input — the server's clock remains the sole source of "when" — but it is what makes "clocking in the next day closes yesterday's shift as `LUPA_PULANG`" and "a clock-out just past midnight" provable in `presensi_usecase_test.go` without a real clock crossing either boundary.
- **Route guards split in two directions, both departures from this table's norms.** The two buttons and one's own history (`/masuk`, `/pulang`, `/saya/hari-ini`, `/saya`) carry **no `RequireRole` at all**, the `auth/me` / `auth/me/password` tier — everybody attends for themselves whatever their role, and a session that has not switched context yet must still be able to tap. Everything showing **other people's** attendance (`/presensi`, `/presensi/rekap`, the `PATCH`) is **`SUPERADMIN`**, a deliberate departure from "reads are open to any authenticated caller": that rule was written for goods and prices, and when somebody arrived is their colleague's personal data. The day an HR role exists, it is the right owner.
- **Isu #12 fase 6 scoping lands on `presensi.id_unit_kerja` itself** — no join to `ruang` and no extra read column, unlike the six modules that needed one. Every presensi read is list-shaped, so rows outside the active unit are omitted silently and nothing 404s for scoping. **`GET /presensi/saya` is not scoped at all** — your own history is yours, including days spent at another unit — and that is the one read in this project where leaving the scope off is the decision rather than the omission, so it has its own test in `fase6_read_scope_test.go`.
- **`created_by`/`updated_by` are deliberately absent** (unlike every master slice since isu #23): `id_user` already names whose row it is and `dikoreksi_oleh` names who changed it. A generic audit column here would only repeat one of the two.
- Employees are `users`, not a new master table — everyone who can press the button already needs an account to log in at all. Validated by the foreign key alone, the `pemakaian.id_pemohon` precedent.
- **No lateness judgement, deliberately, even though the shift hours are known.** Judging needs a per-unit schedule master, and that master has to be versioned like `product_harga_jual` if working hours ever change mid-month — otherwise last month's recap is re-judged with the new hours. Until it exists the responses carry **no lateness field at all**: better to not answer than to answer a zero that reads as "on time".
- **The office-network restriction is not part of this module and must not look like it is.** No subnet check in the usecase; if one is ever added, it belongs in middleware.
- **The daily sweep is the worker's third job, `sapuan-lupa-pulang`** (`presensi.sapuan_interval`, default `24h`, `PresensiUseCase.SapuanLupaPulang`), the same session-level `pg_advisory_lock`-on-a-`*sql.Conn` shape `BersihkanYatim` and `PeriksaRantaiSaldo` already use (`lockSapuanPresensi`, distinct from both their lock keys). It marks `LUPA_PULANG` every `BUKA` row whose `tanggal` has already passed — **per date, never per shift**: a `PAGI` row still `BUKA` at eight the same evening is left alone, because the person may genuinely still be there and a `MALAM` tap already handles that correctly. It repairs a known fact (nobody tapped `Pulang`) rather than reporting an unknown one, which is why it does not contradict isu #25's report-never-repair stance — there a mismatch means a bug with no honest fix from outside the trigger; here the fact is certain and `LUPA_PULANG` records exactly it, no number invented.
- `truncateMaster`: `presensi` points at `users` and `unit_kerja` and nothing points at it, so it goes before both, alongside `dokumen` and `periode`.
...
#### Bacaan atas `kartu_stok` (isu #22)

Five reads, none of them a module: `kartu_stok_riwayat`, `stok_minimum`, and three `laporan_*`. All five follow `riwayat_beli`'s shape — no table, no migration, no number stored anywhere; everything is computed at read time. **No migration exists for this issue at all**, by design.

- **`GET /product/{id}/kartu-stok`** (`KartuStokRepository.Riwayat`) is the first read of `kartu_stok` as a ledger rather than only a tail balance — what answers "why is the balance what it is". `id_ruang` is **required**, unlike `GET /product/{id}/stok`: the balance chain is partitioned by `(id_barang, id_ruang)`, and a history mixing rooms would show a running balance that never existed on any shelf. **Ordered by `id` ascending** — the one list in this API that is not newest-first, because a ledger reads top to bottom, and because the chain's own build order is `id`, not date; a reversal dated `time.Now()` can sit later in the chain than its date alone would suggest. `ref_table` + `ref_id_transaksi` are translated into `nomor_dokumen` by a `CASE` naming every writer `kartu_stok` has today, each branch a primary-key lookup — the same "only a literal already in the query reaches SQL" discipline `RefTableDokumen` uses, applied without needing a map at all since every branch is hardcoded. `id_kartu_stok_asal` non-nil is how a client tells a reversal from an ordinary posting without guessing from `jenis_transaksi`, which the two share.
- **`GET /product/stok-minimum`** (`KartuStokRepository.StokMinimum` + `SaldoPerRuangBatch`) is `riwayat_beli`'s natural pair — that answers who to buy from and at what price, this answers what needs buying at all. `stok_minimum = 0` **never appears**: 0 is the column's default, meaning "never configured", not "may run out" — filtered in SQL, not by the caller. Threshold is `total <= stok_minimum`, never `<`: reaching the reorder point **is** the signal, not one step past it. The comparison sums every room in the caller's active `unit_kerja` (isu #12 fase 6 applied here too); `id_ruang` narrows to one room, for a warehouse deliberately kept near-empty because its stock lives in a shop. Ordered worst-first (`stok_minimum - total` descending), a work queue like `GET /supplier/{id}/utang`. The per-room breakdown behind each flagged product is one batch query for the whole page, never one per row — the same anti-N+1 shape POS established for balances.
- **Three `laporan_*` reads, none belonging to any single module's resource** — a room's inventory value, a month's margin, and a movement recap are cross-cutting, so `LaporanUseCase`/`LaporanController` exist only to give them somewhere to hang, borrowing `KartuStokRepository` and `PenjualanRepository` the way `ProductUseCase` borrows `PembelianRepository`.
  - `GET /laporan/nilai-persediaan` sums `nilai_akhir` off the last row of every `(barang, ruang)` chain, grouped by room. **`ruang.is_aktif` is never filtered** — a retired room still holding stock still holds its value, and hiding it would misstate the balance sheet the report exists to state.
  - `GET /laporan/laba-kotor` is `SUM(total) - SUM(total_hpp)` over **POSTED** `penjualan`, grouped by month of `tanggal`. **The one report here reading a document table, not `kartu_stok`** — and that is correct, not an inconsistency: `total_hpp` is already a snapshot copied from `kartu_stok`'s own `RETURNING` at posting and frozen there, so re-deriving it from `kartu_stok` would pay for a second, more expensive query to reach the same number. `retur_penjualan` does not exist yet, so nothing here reduces a month's margin for a sale that later came back — wired in as a documented gap, not silently.
  - `GET /laporan/pergerakan` sums `stok_masuk`/`stok_keluar` per `(barang, ruang, jenis_transaksi)` in a date range. **This is where the reversal-date trap becomes visible rather than hidden**: the range filters `tanggal_transaksi`, so a document posted in one month and cancelled the next surfaces in the recap covering the cancellation, never the recap covering the original posting (isu #6) — exactly the rule "anything reporting per period must read `kartu_stok`, never the document status" made concrete. Pinned by a dedicated test that posts a document dated two months back, cancels it "today", and asserts the reversal lands in today's recap and the original stays in its own.
- **`GET /laporan/kesehatan-stok` (isu #37)** is a sixth read on the same material: one 0–100 score for the active unit, never returned without its four components (`KETERSEDIAAN` 35, `STOK_MATI` 25, `AKURASI_OPNAME` 25, `CAKUPAN_MINIMUM` 15) and their raw figures. SQL in `kesehatan_stok_repository.go` (methods on `KartuStokRepository`, two queries, one `REPEATABLE READ` read-only tx); arithmetic pure in `kesehatan_stok_skor.go` with an internal test, the `pembelian_alokasi.go` shape. Decisions that are easy to undo by accident:
  - **Weights, the 90-day windows, and the status thresholds are Go constants, not config** — a formula differing per deployment makes outlets incomparable.
  - **`KETERSEDIAAN`'s population deliberately differs from `stok_minimum`'s**: a product must have a `kartu_stok` row in scope. `stok_minimum` is global per product, so without that an outlet is marked `habis` daily for items it never carries. The *thresholds* are identical (`<=`), so everything scored `menipis`/`habis` is also on the stok-minimum list.
  - **Stock death is decided per product across the active unit, not per `(barang, ruang)` chain** — a refinement of the issue text. Demand is `PENJUALAN`/`PEMAKAIAN` anywhere in the unit (a warehouse feeding a selling shop is not dead); `MUTASI_KELUAR` and reversals (`id_kartu_stok_asal IS NOT NULL`) are never demand; a `MUTASI_MASUK` is an arrival only from another unit, so an internal transfer cannot reset goods' age. `id_ruang` narrows which stock is valued, never where demand is looked for. **`RETUR_PENJUALAN` must stay out of the demand list** once that module exists.
  - **A room holding stock with no POSTED opname in the window scores 0**, not skipped — skipping rewards never counting. Surplus and deficit add as absolute values; `stok_so IS NULL` lines are out of both sums.
  - **Null means "not applicable", 0 means "bad"**: a component with nothing to judge is null and the unit score renormalises over the rest; no applicable component at all → `skor: null`. The unit score is taken from exact component values and rounded once — never from the rounded components.
- **Isu #12 fase 6 scoping extends to all five**, list-shaped throughout: a room outside the caller's active `unit_kerja` is silently omitted, never 404 — there is no single resource identity here to 404 against, unlike a `Get`. `GET .../kartu-stok` still 404s for an `id_ruang` that names no room at all; a room that exists but sits outside the active unit answers an empty page instead.
