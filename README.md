...
psql "$DSN" -f db/seeder_postgres/005_product.sql
```
...
### PPN keluaran: nominal yang diketik, bukan tarif yang dihitung

Migrasi `000027` menambah satu kolom, `penjualan.ppn`, dan bentuknya **cermin `pembelian.ppn` sejak migrasi `000005`**: nominal rupiah yang diketik pemanggil, bukan persentase yang dihitung server. Tarif tinggal di kasir; yang dibekukan dokumen adalah uangnya — angka yang harus tetap benar ketika tarif nasional berubah tahun depan, sama seperti setiap angka uang lain di proyek ini. Nol berarti nota yang memang tidak memungut PPN.

Sifatnya **exclusive**: `total = subtotal - diskon_nota + ppn + pembulatan`, rumus yang sama persis dengan `pembelian`. Harga di `product_harga_jual` adalah DPP-nya, dan piutang nota `KREDIT` ikut naik sebesar pajaknya — memang itu yang ditagih, jadi `plafon_kredit`, `sisa_piutang`, dan `status_pembayaran` semuanya bekerja terhadap angka ber-PPN tanpa perlu diubah.

**Tidak ada pasangan `ppn_dikreditkan` seperti di pembelian, dan itu bukan kelalaian.** Di sisi pembelian bendera itu memutuskan apakah PPN masukan ikut menjadi harga pokok persediaan. PPN keluaran tidak pernah punya pilihan itu: ia utang ke negara, bukan biaya, tidak pernah menyentuh HPP, dan karena itu tidak pernah menyentuh `kartu_stok`. Kolom bendera di sini cuma akan jadi pertanyaan yang jawabannya selalu sama.

**Konsekuensinya sampai ke laporan laba kotor, dan di sanalah ia harus dikeluarkan lagi.** `GET /laporan/laba-kotor` kini menghitung `SUM(total - ppn) - SUM(total_hpp)`: PPN yang dipungut di kasir adalah uang negara yang kebetulan lewat di nota, bukan pendapatan toko, dan membiarkannya di dalam omzet akan melebihkan margin tiap bulan persis sebesar pajaknya. Angkanya tetap dilaporkan sendiri sebagai `total_ppn`, jadi `total_penjualan + total_ppn` masih sama dengan jumlah `total` notanya.
...
## Saldo awal: stok pembukaan, harga yang diketik, dan pagar sekali seumur hidup

Isu #43. Sebuah unit kerja yang baru bergabung sudah memegang barang di raknya sebelum baris pertama masuk ke sistem ini, dan tidak ada satu pun jalan jujur untuk memasukkannya: `stok_opname` tidak bisa (`id_kartu_stok_cutoff` `NOT NULL` — barang tanpa riwayat tidak punya apa pun untuk ditunjuk), `pembelian` palsu melahirkan utang kepada supplier yang tidak berutang dan meracuni `riwayat_beli`, dan `mutasi` menuntut ruang sumber yang sudah memegang stoknya. Ini dokumen **kedelapan** yang menulis `kartu_stok`. Migrasi `000030` menambah **satu** nilai enum, `jenis_transaksi = 'SALDO_AWAL'` — `ALTER TYPE ... ADD VALUE` yang tidak bisa dibatalkan (down migration menyatakannya dan meninggalkan nilainya, preseden `000013`) — plus tabel `saldo_awal`/`saldo_awal_detail`.

### Satu-satunya dokumen yang harga pokoknya diketik

Di mana pun juga HPP diturunkan (`pembelian`), disalin dari dokumen asal (`penerimaan_susulan`, `retur_pembelian`), dibaca dari `RETURNING` (`mutasi`, `pemakaian`, `penjualan`), atau diambil dari rata-rata ruang itu sendiri (`stok_opname` surplus). Migrasi adalah satu-satunya keadaan yang memang **tidak punya** dokumen asal untuk disalin — itulah seluruh alasan dokumen ini berbahaya, dan kenapa yang menentukan bukan alur kerjanya melainkan pagarnya. `nilai_masuk = qty_input × harga_satuan_input`, dihitung dari angka yang diketik dan dibulatkan sekali (`math/big.Rat`); `harga_pokok_satuan_dasar` hanya diturunkan supaya baris bisa dibaca per satuan dasar, nilainya tidak lewat pembagian itu — 3 DUS × 100 adalah `300.00`, bukan 36 × 8.3333 = `299.99`. **Harga nol ditolak dengan sengaja**: barang gratis akan menyeret rata-rata bergerak ruang itu ke bawah secara permanen; bila memang perlu, itu `pembelian` dengan diskon penuh.

Tidak ada `id_supplier`, `no_faktur`, atau kolom uang terutang, dan tidak boleh ditambahkan "untuk berjaga-jaga": begitu ada satu saja, modul ini menjadi `pembelian` kedua dengan pagar yang lebih longgar.

### Pagar: satu `(barang, ruang)` seumur hidup

`POST .../posting` ditolak **409** bila `kartu_stok` sudah punya baris **apa pun** untuk pasangan itu — dari `pembelian`, dari `mutasi`, dari `saldo_awal` sebelumnya, **dan baris pembaliknya**. Pesannya menyebut `kode_barang`, nama ruang, dan `stok_opname` sebagai jalan yang benar; semua pelanggar disebut sekaligus (`KartuStokRepository.PernahBergerak`, satu query `unnest` untuk seluruh keranjang).

- **Dibaca setelah kunci, bukan sebelum.** Urutan di `Posting` seragam dengan seluruh proyek — `periode` → `ruang` (`LockShared`) → `KunciSaldo` — baru `PernahBergerak`, baru loop insert. Dua draft yang mengklaim pasangan sama keduanya membaca "belum ada riwayat" kalau dibaca sebelum kunci, lalu keduanya menulis. `TestSaldoAwalDuaPostingBersamaanHanyaSatuYangLolos` mematoknya.
- **Diperiksa dua kali**, mengikuti pola kuota `penerimaan_susulan`: di `Create`/`PUT .../detail` semata untuk pesan yang lebih dini, dan lagi di `Posting` — itu yang menentukan. Dua draft boleh sama-sama mengklaim pasangan yang sama; draft bukan posting.
- **Saldo awal yang salah ketik tidak bisa diulang, dan itu disengaja.** Pembatalan dari `POSTED` menambah baris pembalik (`kartu_stok` append-only), sehingga pasangan itu menjadi punya riwayat dan pintunya tertutup selamanya. Ini bukan efek samping yang perlu diperbaiki — ini pagarnya bekerja. Koreksinya `stok_opname`, yang sejak baris pembalik ada punya `id_kartu_stok_cutoff` untuk ditunjuk dan rata-rata bergerak yang terdefinisi. Melonggarkan pagar menjadi "abaikan baris yang ditulis modul ini sendiri" ditimbang dan ditolak: itu satu celah permanen demi kenyamanan hari pertama, di dokumen yang menciptakan nilai.
- `saldo_awal_detail_baris_uidx` unik pada `(id_saldo_awal, id_product)` — wajib, berbeda dari `mutasi`/`pemakaian` yang sengaja tidak punya. Di sana kuotanya saldo ruang; di sini "tepat satu, selamanya", sehingga dua baris produk sama akan sama-sama lolos pemeriksaan lalu sama-sama menulis (argumen `penerimaan_susulan_detail_baris_uidx`). Usecase menangkapnya lebih dulu supaya pesannya menyebut barisnya.

### Alur status: penuh, karena dokumen ini menciptakan nilai dari tidak ada

`DRAFT → DIAJUKAN → POSTED → BATAL`, dengan `DIAJUKAN → DRAFT` pada penolakan (`alasan_tolak` wajib) — mengikuti `pembelian` dan `stok_opname`, bukan `mutasi`/`penjualan` yang membuang tahap pengajuan. Alasan `mutasi` membuangnya (total stok dan total nilai tidak bergerak) persis terbalik di sini. `INVENTARIS` mengetik dan mengajukan; `SUPERADMIN` menolak, memposting, dan membatalkan. `alasan` header wajib dan tidak bisa dikosongkan lewat `PATCH` — satu-satunya catatan mengapa nilai persediaan diciptakan tanpa dokumen di belakangnya.

`Batal` dari `DRAFT`/`DIAJUKAN` adalah perubahan status murni (tidak ada baris `kartu_stok` yang pernah ditulis, jadi pasangannya tetap terbuka). Dari `POSTED`: baris pembalik bertanggal hari ini, `NilaiMasuk` `"0"` eksplisit — jumlah selalu kembali seimbang, **nilai tidak selalu**, karena antara posting dan pembatalan barang saldo awal sudah tercampur ke rata-rata bergerak. Ditolak 400 bila barangnya sudah keluar dari ruang (obatnya `pemakaian`/`penjualan`/`mutasi` yang sesuai).

### Hal lain yang perlu diketahui

- **Baris `kartu_stok` bertanggal `tanggal` dokumen, bukan `now()`** — tanggal cutover adalah fakta tentang rak pada saat itu, argumen `ts_cutoff` pada `stok_opname`. Konsekuensinya posting ditolak 400 bila periode yang memuat `tanggal` sudah `TUTUP`.
- Memanggil `periksaKatalogRuang` (di `Create`/`PUT .../detail` **dan lagi** di `Posting`), `periksaRuangBeku` (`Posting` dan `Batal`), `periksaPeriode`, `periksaRuangUnitAktif`, dan `RuangRepository.LockShared` sebelum `KunciSaldo` — semua yang wajib bagi modul yang menulis `kartu_stok` dari baris yang diketiknya sendiri. `HasRef` sebagai jaring terakhir agar satu dokumen tidak bisa diposting dua kali.
- Ikut disaring unit_kerja aktif (isu #12 fase 6): `Get` di luar unit aktif menjawab 404, `List` menghilangkan barisnya.
- **Lampiran didukung** (`saldo_awal` di `repository.RefTableDokumen`, tanpa migrasi): karena harga pokoknya diketik, berita acara serah terima atau lembar hitung bertanda tangan adalah satu-satunya bukti eksternal di belakang angka itu. Mewajibkannya sebelum `Ajukan` sengaja di luar cakupan — tidak ada modul lain yang mewajibkan lampiran.
- **`riwayat_beli` tidak boleh menyentuh modul ini.** Query-nya berjalan di atas `pembelian_detail` jadi secara alami aman; harga saldo awal adalah taksiran, bukan harga yang pernah dibayar, dan memasukkannya ke sana merusak satu-satunya bacaan yang dipakai menawar dengan supplier. `laporan/pergerakan` menampilkan `SALDO_AWAL` sebagai kelompoknya sendiri — yang diinginkan. Di `laporan/kesehatan-stok`, baris `SALDO_AWAL` adalah **kedatangan** untuk umur `STOK_MATI` dan tidak pernah permintaan.
- Di luar cakupan: impor massal CSV/Excel (`PUT .../detail` sudah mengganti seluruh baris sekaligus), saldo awal utang/piutang (isu terpisah), dan revaluasi persediaan.
- Prefix nomor `SA`, dikunci ke unit pemilik ruang dokumennya.
...
Pasangan alami [riwayat harga beli](#riwayat-harga-beli-pengganti-purchase-order): yang itu menjawab beli ke siapa dan berapa harganya, ini menjawab apa yang harus dibeli sama sekali. `stok_minimum = 0` tidak pernah muncul — nol nilai default kolom itu, artinya belum diatur, bukan boleh habis. Ambangnya `total_stok <= stok_minimum`, bukan `<`: menyentuh titik pesan ulang **adalah** saatnya memesan. `total_stok` dijumlahkan lintas seluruh ruang dalam `unit_kerja` aktif pemanggil, dengan rincian per ruang di `per_ruang`; `id_ruang` mempersempit ke satu ruang saja. Hanya barang `is_aktif`, diurutkan dari yang paling parah — pola yang sama dengan `GET /supplier/{id}/utang`. `search` menyaring `nama` dan `kode_barang` **di SQL**, bukan di halaman yang sudah termuat: daftar ini berhalaman, jadi kolom cari di sisi klien cuma pernah menemukan barang yang kebetulan ada di halaman itu — yang dicari di halaman dua tidak akan pernah ketemu.
...
- **`GET /api/v1/laporan/laba-kotor?dari=&sampai=`** — `SUM(total - ppn) - SUM(total_hpp)` atas nota `penjualan` **POSTED**, per bulan. **PPN keluaran dikeluarkan dari omzet** — uang negara yang cuma lewat di nota, bukan pendapatan toko — dan dilaporkan sendiri sebagai `total_ppn`, jadi `total_penjualan + total_ppn` tetap sama dengan jumlah `total` notanya. Satu-satunya di antara kelimanya yang membaca dokumen dan bukan `kartu_stok`, dan itu benar: `total_hpp` sudah disalin dari `RETURNING kartu_stok` saat posting dan dibekukan di sana, membacanya ulang dari kartu stok cuma akan membayar query yang lebih mahal untuk angka yang sama. Nota `BATAL` dikeluarkan; `retur_penjualan` belum ada sehingga belum ada yang mengurangi laba kotornya — begitu modul itu ada, kreditnya masuk di sini.
...
## Presensi karyawan: satu tombol masuk, satu tombol pulang, dua shift per hari

Aplikasi ini sudah selalu ada di handphone karyawan dan hanya bisa dibuka dari jaringan kantor — pembatasan jaringannya di luar repo ini dan **bukan bagian modul ini**. Yang ditambahkan isu #40 adalah pencatat siapa datang jam berapa dan pulang jam berapa, dengan dua tombol, tanpa form.

```bash
POST /api/v1/presensi/masuk            # tanpa body sama sekali
POST /api/v1/presensi/pulang           # tanpa body sama sekali
GET  /api/v1/presensi/saya/hari-ini    # apa yang harus digambar tombolnya
GET  /api/v1/presensi/saya             # riwayat sendiri, berpaginasi
```

| Shift | Jam kerja |
|---|---|
| `PAGI` | 07:00 – 17:00 WIB |
| `MALAM` | 18:00 – 21:00 WIB |

Satu orang boleh hadir di **kedua** shift pada hari yang sama, dan yang begitu dibayar lebih tinggi. **Penggajiannya sendiri di luar cakupan** — yang disediakan modul ini hanya rekap yang bisa menjawab pertanyaan itu nanti tanpa dihitung ulang.

Ini modul pertama di proyek ini yang **tidak menyentuh `kartu_stok` maupun uang**: tidak ada `periode`, tidak ada pembekuan ruang, tidak ada `document_counter`, tidak ada `big.Rat`. Yang tersisa dari kebiasaan proyek ini hanya yang memang relevan — identitas datang dari sesi, kosakata dijaga CHECK, cache tidak pernah diisi dari form, dan setiap koreksi meninggalkan jejak.

### Shift disimpulkan server, bukan dipilih karyawan

Satu konstanta batas, **`batasShiftWIB = 17:30`**, memisahkan keduanya: tap masuk sebelum itu `PAGI`, sisanya `MALAM`. Satu tombol tetap satu tombol; menambah pilihan shift berarti menambah cara salah pilih, dan berarti seseorang bisa mengaku hadir di shift malam sambil menekan tombolnya pukul delapan pagi.

**Satu batas, bukan dua jendela.** Jendela `07:00–17:00` dan `18:00–21:00` meninggalkan tiga celah — sebelum tujuh pagi, jeda 17:00–18:00, dan setelah sembilan malam — dan setiap celah adalah tap yang harus ditolak padahal orangnya benar-benar berdiri di sana. Batas tunggal tidak punya celah: setiap jam memetakan ke tepat satu shift. Konsekuensi yang diterima sadar — tap pukul 03:00 tercatat `PAGI` yang sangat awal, tap pukul 23:00 tercatat `MALAM` yang sangat telat — keduanya tetap **tercatat**, dan koreksi ada untuk yang benar-benar salah shift.

Batas ini **memindahkan uang**: 17:29 satu shift, 17:31 shift lain. Karena itu ia satu konstanta bernama di satu tempat, fungsinya murni dan diuji tanpa database (`presensi_shift.go`, bentuk yang sama dengan `pembelian_alokasi.go`), dan **jam server yang meleset adalah satu-satunya hal yang bisa menggesernya** — klien tidak pernah menyimpulkan shift sendiri. `GET /presensi/saya/hari-ini` yang memutuskan tombol mana digambar, karena di sekitar 17:30 jam handphone yang meleset beberapa menit sudah cukup untuk menaruh kehadiran seseorang di shift yang salah.

**Tombol pulang tidak pernah menyimpulkan ulang shift dan tidak membandingkan jamnya dengan jam selesai shift.** Shift ditentukan sekali, saat masuk; lembur dan pulang telat bukan urusan modul ini untuk dinilai. Ia menutup baris `BUKA` milik pemanggil **apa pun tanggal dan shiftnya** — bukan "baris hari ini" — supaya tap pulang lewat tengah malam tetap menutup shift malam kemarin, bukan menjawab "belum presensi masuk".

### Dua indeks unik, dan keduanya tidak boleh ditukar kuncinya

`presensi_user_tanggal_shift_uidx` unik pada **`(id_user, tanggal, shift)`**. Perhatikan `shift` di dalamnya: tanpa itu orang yang hadir pagi **dan** malam tertolak di ketukan keduanya, dan justru hari yang paling bernilai bagi penggajian yang jadi tidak bisa dicatat sama sekali.

`presensi_terbuka_uidx` unik **parsial** pada `(id_user) WHERE status = 'BUKA'` — dan `shift` **sengaja tidak ada** di kuncinya: seseorang tidak bisa berada di dua shift sekaligus. Indeks ini melakukan dua hal sekaligus, persis seperti `stok_opname_ruang_terbuka_uidx`: menghalangi dua shift terbuka bersamaan pada satu orang, dan membuat "baris mana yang ditutup tombol pulang" punya tepat satu jawaban — tanpanya `Pulang` harus menebak.

Tap masuk kedua di shift yang **sama** menjawab **409 menyebut jam yang sudah tercatat** — bukan baris kedua, dan bukan pula 200 diam-diam. Handphone di jaringan buruk akan mengirim tap yang sama dua kali, dan jawabannya harus cukup bagi klien untuk menggambar keadaan yang benar tanpa berpura-pura baru mencatat sesuatu. Pemeriksaan di Go cuma untuk pesan yang lebih ramah; yang benar-benar menjamin satu baris per shift adalah indeksnya, sesuai aturan "check-then-insert tidak pernah menjamin keunikan".

### `LUPA_PULANG`: mengarang jam pulang adalah satu-satunya kebohongan yang dihindari modul ini

Kalau pemanggil menekan tombol masuk sementara masih ada shift lain yang terbuka — lupa tap pulang kemarin, atau lupa tap pulang shift pagi lalu langsung lanjut shift malam — baris lama itu ditandai `LUPA_PULANG` **di transaksi yang sama**, dengan `jam_pulang` dibiarkan `NULL` selamanya. Satu aturan ini menangani dua kejadian sekaligus.

Setiap godaan mengisinya otomatis (jam selesai shift, tanggal + 10 jam, waktu tap terakhir) menghasilkan angka yang terbaca seperti fakta padahal tebakan — dan di modul yang ujungnya penggajian, tebakan itu jadi rupiah. Jalur memperbaikinya adalah `PATCH` berjejak, bukan penebakan otomatis.

Baris yang tidak pernah ditutup tombol lagi — orang yang resign, cuti panjang, atau sakit seminggu — dibereskan **sapuan harian di worker**, job ketiga di `cmd/worker` (`presensi.sapuan_interval`, default `24h`): setiap baris `BUKA` yang tanggalnya sudah lewat ditandai `LUPA_PULANG`, tanpa pernah mengarang jamnya. Sapuannya per **tanggal**, bukan per shift — baris `PAGI` yang masih `BUKA` pukul delapan malam di hari yang sama tetap dibiarkan, karena orangnya mungkin masih di tempat dan tap masuk shift malam sudah menangani kasus itu dengan benar.

### Identitas datang dari sesi, dan di sini itu bukan sekadar kebiasaan

`id_user` diambil dari `middleware.SessionFrom` dan **tidak ada di DTO mana pun** — bukan `json:"-"` lalu ditimpa seperti `ActorID`, melainkan memang tidak ada fieldnya. Kalau ia bisa dikirim klien, satu karyawan bisa mengabsenkan temannya yang belum sampai kantor, dan seluruh modul ini kehilangan alasannya untuk ada.

`id_unit_kerja` adalah **snapshot** konteks aktif sesi saat menekan tombol masuk. Diturunkan saat dibaca akan menulis ulang sejarah: grant seseorang bisa berpindah unit bulan depan, dan rekap bulan lalu ikut berpindah bersamanya. Sesi tanpa konteks aktif tetap boleh absen — barisnya ber-`id_unit_kerja` NULL, dan konsekuensinya baris itu hanya terlihat oleh pemanggil bergrant global.

`ip_masuk`/`ip_pulang` diisi dari `ctx.IP()` apa adanya dan **belum membuktikan apa pun**: proyek ini tidak punya konfigurasi trusted-proxy di mana pun — peringatan yang sama sudah ditulis untuk pembatasan laju login. Kolomnya diisi sekarang supaya berguna begitu setelan itu ada, bukan supaya dipercaya hari ini.

### Rekap: melaporkan, tidak memutuskan

`GET /presensi/rekap` (`SUPERADMIN`) mengelompokkan per `(karyawan, bulan)` dan **memisahkan shift, bukan menjumlahkannya**: `hari_pagi_selesai`, `hari_malam_selesai`, `hari_dua_shift`, `hari_lupa_pulang_pagi`/`hari_lupa_pulang_malam` (dihitung terpisah, tidak pernah dilebur ke angka SELESAI), dan `total_menit_kerja_pagi`/`total_menit_kerja_malam`.

`hari_dua_shift` angka tersendiri, bukan sesuatu yang bisa diturunkan klien — `hari_pagi_selesai + hari_malam_selesai` menghitung hari itu dua kali dan tidak bisa menyebut hari mana yang bertindih. Ia satu agregasi bertingkat di dalam **satu query untuk seluruh halaman**, bukan loop di Go: bentuk anti-N+1 yang sama dengan `SaldoPerRuangBatch`.

Yang sengaja tidak dilakukannya: memutuskan apakah hari `LUPA_PULANG` tetap dihitung hadir. Itu pertanyaan penggajian, dan menjawabnya di sini berarti mengunci kebijakan gaji ke dalam query rekap sebelum kebijakannya sendiri ditulis.

### `durasi_kerja_menit`: satu-satunya angka turunan yang sengaja bukan snapshot

Dihitung saat dibaca dari `jam_pulang - jam_masuk`, **tidak pernah disimpan**. Angka uang di proyek ini dibekukan karena sumbernya bergerak (rata-rata bergerak, sisa piutang); di sini justru sebaliknya — kedua operannya ada di baris yang sama, dan koreksi yang menggeser salah satu jamnya **harus** menggeser durasinya juga, atau angka tersimpan akan diam-diam bertentangan dengan dua kolom di sebelahnya. Baris `BUKA` dan `LUPA_PULANG` menjawab `null`, bukan `0` — nol berarti masuk dan pulang di detik yang sama, `null` berarti pertanyaannya belum punya jawaban, bedanya sama dengan `stok_so = NULL` yang bukan nol di `stok_opname`.

### Koreksi yang meninggalkan jejak

Orang lupa menekan tombol pulang; itu kejadian harian, bukan kasus tepi. Yang tidak boleh terjadi adalah baris hasil koreksi tidak bisa dibedakan dari baris hasil klik.

`PATCH /api/v1/presensi/{id}` (`SUPERADMIN`) mengoreksi `jam_masuk`, `jam_pulang`, dan/atau `shift`. Setiap patch yang berhasil mengisi `dikoreksi_oleh`, `ts_koreksi`, dan menandai `sumber_masuk`/`sumber_pulang` yang bersangkutan `KOREKSI`, plus `alasan_koreksi` yang **wajib** dan tidak bisa dikosongkan — satu-satunya catatan kenapa jam seseorang berubah. Kebijakan, bukan constraint; presedennya `keterangan_selisih` dan `retur_pembelian.alasan`.

**`shift` boleh dikoreksi tapi tidak pernah disimpulkan ulang dari `jam_masuk` yang baru.** Koreksi jam dan koreksi shift adalah dua keputusan terpisah; menautkannya berarti membetulkan salah ketik satu menit diam-diam memindahkan orang ke shift lain. `jam_masuk`/`jam_pulang` dikirim sebagai jam dinding WIB (`"06:45"`) dan ditempelkan pada `tanggal` baris itu sendiri, jadi koreksi tidak bisa menyeberang hari — memindahkan presensi ke hari lain bukan koreksi, dan `id_user`/`tanggal` memang tidak ada di DTO-nya.

**`jam_pulang` boleh diisi atau dipindah, tapi tidak boleh dikosongkan kembali.** Membuka kembali shift yang sudah `SELESAI` akan mengaktifkan lagi `presensi_terbuka_uidx` untuk hari yang sudah lewat, dan orangnya tidak bisa absen lagi besok pagi. **`status` selalu dihitung ulang dari kolomnya, tidak pernah diterima dari form** — `jam_pulang` terisi berarti `SELESAI`; mengoreksi baris `LUPA_PULANG` dengan mengisi `jam_pulang`-nya membuatnya `SELESAI` dengan sendirinya.

### Hal lain yang perlu diketahui

- **WIB dipakai sebagai offset tetap UTC+7** (`time.FixedZone`), bukan `time.LoadLocation` — keputusan yang sudah diambil di `tanggalHargaJual` dan alasannya berlaku utuh: Indonesia bagian barat tidak punya DST, dan tidak ada yang bergantung pada tzdata terpasang di container. Isu ini mengangkatnya jadi satu `zonaWIB` di `shared.go` yang dipakai keduanya — di modul ini zonanya menentukan **dua** hal sekaligus, tanggal dan shift, jadi satu definisi yang meleset salah dua kali.
- **Karyawan adalah `users`, bukan tabel master baru.** Setiap orang yang bisa menekan tombolnya sudah harus punya akun untuk login sama sekali; tabel `karyawan` terpisah akan langsung melahirkan pertanyaan "baris mana milik akun mana". Presedennya `pemakaian.id_pemohon` — divalidasi foreign key saja.
- **Tidak ada penilaian "terlambat"**, walaupun jam shiftnya sudah diketahui. Menilai butuh master jadwal per unit kerja, dan jadwal itu harus berversi seperti `product_harga_jual` kalau jam kerja pernah berubah di tengah bulan — kalau tidak, rekap bulan lalu dinilai ulang dengan jam yang baru. Selama itu belum ada, responsnya **tidak memuat field keterlambatan sama sekali**: lebih baik tidak menjawab daripada menjawab nol yang akan dikira "tepat waktu".
- **Pembatasan jaringan kantor bukan bagian modul ini dan tidak terlihat seperti bagian modul ini.** Tidak ada pemeriksaan subnet di usecase. Kalau nanti ada, tempatnya middleware.
- Di luar cakupan juga: GPS/geofence/selfie (ruang kantor sudah jadi pagarnya; menambahkannya mengubah "klik tombol" jadi form), penjadwalan shift di muka (modul ini mencatat siapa yang **benar-benar hadir**, bukan siapa yang **seharusnya**), serta cuti, izin, sakit, dan lembur — semuanya dokumen dengan persetujuan, bukan tombol.
...
| `saldo_awal` — input, edit, ganti baris, ajukan | semua yang login | `SUPERADMIN`, `INVENTARIS` |
| `saldo_awal` — tolak, posting, batal | — | `SUPERADMIN` |
| `periode` (tutup buku) | semua yang login | `SUPERADMIN` |
| `presensi` — tombol masuk/pulang, riwayat sendiri | — | semua yang login, **tanpa role guard** |
| `presensi` — daftar, rekap, koreksi | `SUPERADMIN` | `SUPERADMIN` |
...
**`presensi` terbelah dua, dan dua-duanya menyimpang dari baris lain di tabel ini — ke arah yang berlawanan.** Tombol masuk/pulang dan riwayat sendiri **tanpa role guard sama sekali**, setingkat `auth/me` dan `auth/me/password`: setiap karyawan absen untuk dirinya sendiri apa pun perannya, dan sesi yang belum memilih konteks aktif (`Aktif == nil`, yang ditolak setiap `RequireRole`) tetap harus bisa menekannya. Sebaliknya **setiap bacaan yang menampilkan kehadiran orang lain hanya `SUPERADMIN`**, penyimpangan sadar dari "baca terbuka untuk semua yang login": aturan itu lahir untuk data barang dan harga, sedangkan jam datang seseorang adalah data pribadi rekan kerjanya, dan tidak ada alasan seorang kasir bisa menarik jam masuk seluruh kantor. `GET /presensi/saya` menutup kebutuhan yang sah bagi semua orang. Kalau nanti ada peran HR, dialah pemilik yang benar.
...
Skema lengkap ada di migrasi `000002`–`000008`. **`pembelian`, `penerimaan_susulan`, `retur_pembelian`, `pembayaran_utang`, `mutasi`, `pemakaian`, `penjualan`, `penerimaan_pembayaran`, `stok_opname`, dan `saldo_awal` sudah punya lapisan Go dan memakainya**; retur penjualan belum. Beberapa jaminan ditegakkan database, bukan aplikasi:
...
| `GET` | `/api/v1/product/stok-minimum` | Produk `is_aktif` dengan `total_stok <= stok_minimum`, terparah dulu — `page`, `size`, `search`, `id_ruang` |
...
| `GET` | `/api/v1/laporan/laba-kotor` | `SUM(total - ppn) - SUM(total_hpp)` nota `POSTED`, per bulan — `dari`, `sampai` |
...
| `GET` | `/api/v1/saldo_awal` | List — `page`, `size`, `search`, `status`, `id_ruang`, `tanggal_dari`, `tanggal_sampai`; disaring unit aktif |
| `POST` | `/api/v1/saldo_awal` | Buat draft; `alasan` wajib; `detail` boleh kosong; barang yang sudah punya riwayat di ruang itu ditolak 409 — `INVENTARIS` |
| `GET` | `/api/v1/saldo_awal/{id}` | Detail beserta barisnya; 404 bila di luar unit aktif |
| `PATCH` | `/api/v1/saldo_awal/{id}` | Ubah header — hanya saat `DRAFT`; `alasan` tidak bisa dikosongkan — `INVENTARIS` |
| `PUT` | `/api/v1/saldo_awal/{id}/detail` | Ganti seluruh baris — hanya saat `DRAFT`; satu baris per produk — `INVENTARIS` |
| `POST` | `/api/v1/saldo_awal/{id}/ajukan` | `DRAFT` → `DIAJUKAN` — `INVENTARIS` |
| `POST` | `/api/v1/saldo_awal/{id}/tolak` | `DIAJUKAN` → `DRAFT`, wajib `alasan` — `SUPERADMIN` |
| `POST` | `/api/v1/saldo_awal/{id}/posting` | Tulis satu baris `kartu_stok` masuk per baris; **pagar sekali seumur hidup** dibaca di sini — `SUPERADMIN` |
| `POST` | `/api/v1/saldo_awal/{id}/batal` | Wajib `alasan_batal`; dari `POSTED` menutup pasangan itu selamanya — `SUPERADMIN` |
...
| `POST` | `/api/v1/presensi/masuk` | Tombol masuk — tanpa body, tanpa role guard; shift disimpulkan server |
| `POST` | `/api/v1/presensi/pulang` | Tombol pulang — tanpa body, tanpa role guard; menutup shift `BUKA` milik pemanggil |
| `GET` | `/api/v1/presensi/saya/hari-ini` | Keadaan kedua shift hari ini, untuk menggambar tombolnya |
| `GET` | `/api/v1/presensi/saya` | Riwayat sendiri, berpaginasi — `page`, `size`, `tanggal_dari`, `tanggal_sampai`, `shift`, `status`; tidak disaring unit |
| `GET` | `/api/v1/presensi` | List semua presensi (`SUPERADMIN`) — `page`, `size`, `id_user`, `tanggal_dari`, `tanggal_sampai`, `shift`, `status` |
| `GET` | `/api/v1/presensi/rekap` | Rekap sebulan per karyawan (`SUPERADMIN`) — `page`, `size`, `tahun`, `bulan` |
| `PATCH` | `/api/v1/presensi/{id}` | Koreksi `jam_masuk`/`jam_pulang`/`shift` (`SUPERADMIN`), `alasan_koreksi` wajib |
...
- **Modul `saldo_awal`** (isu #43): stok pembukaan untuk unit yang baru migrasi, dokumen kedelapan yang menulis `kartu_stok` dan satu-satunya yang harga pokoknya diketik — karena itu yang dijaga adalah pagarnya: satu `(barang, ruang)` seumur hidup, dibaca setelah kunci saldo dipegang, termasuk baris pembalik, sehingga saldo awal yang salah ketik dikoreksi lewat `stok_opname`. Alur persetujuan penuh, nilai enum `SALDO_AWAL` baru (migrasi `000030`). `retur_penjualan` (isu #33) kini satu-satunya modul tanpa lapisan Go
...
- **Bacaan atas `kartu_stok`** (isu #22): `GET /product/{id}/kartu-stok` — riwayat pergerakan satu barang di satu ruang, urut `id` naik, dengan `ref_table`+`ref_id_transaksi` diterjemahkan jadi `nomor_dokumen` lewat `CASE`, dan baris pembalik ditandai `id_kartu_stok_asal`; `GET /product/stok-minimum` — barang `is_aktif` dengan `total_stok <= stok_minimum` (bukan `<`), `stok_minimum = 0` tidak pernah muncul, terparah dulu; dan tiga laporan tanpa kolom baru — `GET /laporan/nilai-persediaan` (jumlah `nilai_akhir` per ruang, termasuk ruang pensiun), `GET /laporan/laba-kotor` (`SUM(total - ppn) - SUM(total_hpp)` nota `POSTED` per bulan, satu-satunya yang membaca dokumen bukan `kartu_stok`), dan `GET /laporan/pergerakan` (`stok_masuk`/`stok_keluar` per `(barang, ruang, jenis_transaksi)` dalam rentang tanggal — teruji secara eksplisit menempatkan pembalikan dokumen periode lama di periode pembatalannya, bukan periode dokumennya). Tidak ada migrasi sama sekali di isu ini, sesuai definition of done-nya
...
- **Modul `presensi`** (isu #40): modul pertama yang tidak menyentuh `kartu_stok` maupun uang — dua tombol `POST /presensi/masuk`/`POST /presensi/pulang` tanpa body sama sekali, shift disimpulkan server dari satu konstanta batas `17:30 WIB` lewat fungsi murni yang diuji tanpa database, dan `id_user` yang tidak pernah ada di DTO mana pun supaya tidak ada yang bisa mengabsenkan temannya. Kehadiran dicatat sebagai `jam_masuk`/`jam_pulang` dan sebuah `status` (`BUKA`/`SELESAI`/`LUPA_PULANG`) yang selalu dihitung ulang, tidak pernah diterima dari form. Satu orang boleh hadir dua shift sehari (`presensi_user_tanggal_shift_uidx` berkunci `(id_user, tanggal, shift)`; tap kedua di shift yang sama menjawab 409 menyebut jamnya) tapi tidak pernah dua shift terbuka sekaligus (`presensi_terbuka_uidx`, parsial, tanpa `shift` di kuncinya). Lupa tap pulang menandai baris lama `LUPA_PULANG` dengan `jam_pulang` tetap `NULL` selamanya — tidak pernah mengarang jam — baik lewat tap masuk berikutnya maupun lewat sapuan harian di worker (`presensi.sapuan_interval`, job ketiga). Rekap per `(karyawan, bulan)` memisahkan shift dan melaporkan `hari_dua_shift` sebagai angka tersendiri dalam satu query per halaman — **melaporkan, tidak memutuskan**, karena penggajiannya di luar cakupan. Koreksi (`PATCH /presensi/{id}`, `SUPERADMIN`) selalu menulis `dikoreksi_oleh`/`ts_koreksi`/`sumber_masuk`-`sumber_pulang` yang bersangkutan beserta alasannya, dan tidak pernah menyimpulkan ulang shift dari jam yang dikoreksi. Tombol dan riwayat sendiri tanpa role guard; setiap bacaan atas kehadiran orang lain `SUPERADMIN`
