...
func TestPelangganNullPlafonKreditStaysNull(t *testing.T) {
	...
	created, err := a.pelanggan.Create(ctx(), &model.CreatePelangganRequest{Nama: "Tanpa Plafon"})
	if err != nil {
...
func TestPelangganPatchCanSetAndClearPlafonKredit(t *testing.T) {
	...
	patched := patchPelanggan(t, a, created.ID, `{"plafon_kredit":"2500.75"}`)
	...
	patched = patchPelanggan(t, a, created.ID, `{"nama":"Naik Turun Lagi"}`)
	if patched.PlafonKredit == nil || *patched.PlafonKredit != "2500.75" {
	...
	patched = patchPelanggan(t, a, created.ID, `{"plafon_kredit":null}`)
...
func TestPelangganNegativePlafonKreditIsInvalid(t *testing.T) {
	...
	created, err := a.pelanggan.Create(ctx(), &model.CreatePelangganRequest{Nama: "Plafon Wajar"})
	if err != nil {
		t.Fatalf("create: %v", err)
	}

	request := decodePelangganPatch(t, created.ID, `{"plafon_kredit":"-500.00"}`)
...
func TestPelangganManyNullKodeAreAllowed(t *testing.T) {
	...
	for _, nama := range []string{"Tanpa Kode A", "Tanpa Kode B"} {
		if _, err := a.pelanggan.Create(ctx(), &model.CreatePelangganRequest{Nama: nama}); err != nil {
...
func decodePelangganPatch(t *testing.T, id int64, body string) *model.UpdatePelangganRequest {
	t.Helper()
...
func patchPelanggan(t *testing.T, a *app, id int64, body string) *model.PelangganResponse {
	t.Helper()

	response, err := a.pelanggan.Update(ctx(), decodePelangganPatch(t, id, body))
	if err != nil {
