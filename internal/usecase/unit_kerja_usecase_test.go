...
func TestUnitKerjaPatchRejectsNullOnNotNullColumn(t *testing.T) {
	...
	created, err := a.unitKerja.Create(ctx(), &model.CreateUnitKerjaRequest{Nama: "Unit Nama"})
	if err != nil {
		t.Fatalf("create: %v", err)
	}

	request := decodeUnitKerjaPatch(t, created.ID, `{"nama":null}`)
...
func TestUnitKerjaPatchKodeConflicts(t *testing.T) {
	...
	_, err = a.unitKerja.Update(ctx(), decodeUnitKerjaPatch(t, second.ID, `{"kode":"UNIT-A"}`))
	assertKind(t, err, model.KindConflict)

	// Re-sending its own code is not a conflict: exceptID skips the row itself.
	patched := patchUnitKerja(t, a, second.ID, `{"kode":"UNIT-B"}`)
...
func TestRuangCreateRejectsRetiredUnitKerja(t *testing.T) {
	...
	unit, err := a.unitKerja.Create(ctx(), &model.CreateUnitKerjaRequest{Nama: "Unit Pensiun"})
	if err != nil {
...
func TestRuangResponseIncludesUnitKerjaName(t *testing.T) {
	...
	unit, err := a.unitKerja.Create(ctx(), &model.CreateUnitKerjaRequest{Nama: "Outlet Utama"})
	if err != nil {
		t.Fatalf("create unit: %v", err)
	}
...
func decodeUnitKerjaPatch(t *testing.T, id int64, body string) *model.UpdateUnitKerjaRequest {
	t.Helper()
...
func patchUnitKerja(t *testing.T, a *app, id int64, body string) *model.UnitKerjaResponse {
	t.Helper()

	response, err := a.unitKerja.Update(ctx(), decodeUnitKerjaPatch(t, id, body))
	if err != nil {
